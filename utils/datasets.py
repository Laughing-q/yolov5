...
def load_mask(self, index):
    mask_path = self.mask_fils[index]
...
class LoadImagesAndLabelsAndMasks(Dataset):  # for training/testing
    def __init__(self,
                 path,
                 img_size=640,
                 batch_size=16,
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            segments = self.segments[
                index]  # [array, array, ....], arrat.shape=(num_points, 2), xyxyxyxy
            masks = []
            # print(segments)
            if len(segments):
                for i_s in range(len(segments)):
                    segments[i_s] = xyn2xy(segments[i_s],
                                           ratio[0] * w,
                                           ratio[1] * h,
                                           padw=pad[0],
                                           padh=pad[1])
    ...
    def collate_fn(batch):
        ...
        return torch.stack(img,
                           0), torch.cat(label,
                                         0), path, shapes, torch.cat(masks, 0)
